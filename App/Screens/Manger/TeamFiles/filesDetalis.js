...
export default function FilesDetail(props) {
  const [sportsName, setSportsName] = useState('');
  const [sportsValue, setSportsData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState(['photvisual.pdf','abc.pdf','photvisual.pdf','abc.pdf','photvisual.pdf','abc.pdf','photvisual.pdf','abc.pdf',]);
    ...
    <>
      <SafeAreaView style={{flex: 1, backgroundColor: '#414141'}}>
        ...
        <FlatList
              ...
              renderItem={({item}) => {
                return (
                    <View style={{borderBottomWidth:0.5,borderColor:COLORS.TEXTCOLORS}}>
                        <Text style={{padding:10,color:COLORS.TEXTINPUTBACKCOLOR}}>{item}</Text>
                    </View>
